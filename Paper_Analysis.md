...
Legend: **P**=Prediction, **Th**=Threshold, **D**=Detection, **R**=Recovery, **FG**=Formal Guarantee. ✓ yes · ✗ no · ◐ partial.
...
|---|---|---|---|---|:-:|:-:|:-:|:-:|---|:-:|
| 1 | Adv‑LiDAR | LiDAR | AV/UGV | Attack | ✗ | ◐ | ✗ | ✗ | — | 3 |
| 2 | Anomaly Detection – STL | generic CPS | neither (train) | Detection | ✓ | ✓ | ✓ | ✗ | ◐ STL robustness semantics | 2 |
| 3 | DeLorean | GPS,gyro,accel,mag,baro | UAV+UGV | Detect+Diagnose+Recover | ✓ | ✓ | ✓ | ✓ | ✗ | 3 |
| 4 | ESP Spoofing | MEMS gyro | UGV (car) | Attack+defense | ✓ | ✓ | ✓ | ◐ | ✗ | 2 |
| 5 | RIDS | GPS,IMU/accel | UGV(+UAV) | Detection | ✓ | ✓ | ✓ | ✗ | ◐ Chi‑square hypothesis test | 3 |
| 6 | GAN‑MP | GPS,IMU,cam,LiDAR | UGV | Mitigation | ✓ | ✗ | ✗ | ✓ | ✗ | 2 |
| 7 | Hybrid Monitors | CPS I/O | ADS+APS | Detect+Mitigate | ✓ | ✓ | ✓ | ◐ | ◐ STL specification | 3 |
| 8 | Injected & Delivered | MEMS IMU | mixed embedded | Attack | ✗ | ✗ | ✗ | ✗ | ◐ sampling‑drift theorem | 3 |
| 9 | MSF‑ADV | camera+LiDAR | AV/UGV | Attack | ✗ | ◐ | ✗ | ✗ | — | 3 |
| 10 | SeqRec | CPS sensors | UAV+UGV | Recovery | ✓ | ✗ | ✗ | ✓ | ✗ | 3 |
| 11 | PID‑Piper | GPS,baro,gyro,accel,mag | UAV+UGV | Detect+Recover | ✓ | ✓ | ✓ | ✓ | ✗ | 3 |
| 12 | PyCRA | active sensors | neither | Detect+Resilience | ✓ | ✓ | ✓ | ◐ | ✓ physical‑delay bound + χ² detector | 3 |
| 13 | Real‑time Recovery LQR | CPS sensors | UAV+UGV(sim) | Recovery | ✓ | ✗ | ✗ | ✓ | ✓ reachability analysis | 3 |
| 14 | Nyctea | flight state | UAV | Detect+Rectify(config) | ◐ | ✓ | ✓ | ✓ | ✗ | 2 |
| 15 | Data‑Predictive Recovery | cam,LiDAR,IMU,GPS | UAV+UGV+marine | Recovery | ✓ | ✗ | ✗ | ✓ | ✓ nonlinear reachability (Flow*) | 3 |
| 16 | Recovery Survey | — | all CPS | Survey | ◐ | — | ◐ | ✓ | ◐ (reviews formal methods) | 3 |
| 17 | Robotics Security Survey | — | all robots | Survey | ◐ | — | ✓ | ◐ | — | 2 |
| 18 | DAE‑Crit‑LSTM | position/localization | UGV(multi) | Mitigation | ✓ | ✗ | ✗ | ✓ | ✗ | 3 |
| 19 | Robustness Testing | APS glucose | neither(APS) | Detection robustness | ✓ | ◐ | ✓ | ✗ | ✗ | 2 |
| 20 | SCVMON | gyro,accel,baro,GPS+params | UAV | Detect+Recover | ✓ | ✓ | ✓ | ✓ | ◐ s‑t‑cut completeness proof | 3 |
| 21 | Secure Autonomous CPS | camera,LiDAR,GPS | UGV(Segway) | Prevention | ✗ | ✗ | ✓ | ✗ | ✓ language‑based IFC (Jif) + verified HW | 3 |
| 22 | UAV Attack Classification | gyro,accel,mag,baro,GPS | UAV | Detect+Classify | ◐ | ◐ | ✓ | ✗ | ✗ | 2 |
| 23 | Gecko | IMU/peripherals | UAV+UGV+arm | Recovery(availability) | ✓ | ◐ | ◐ | ✓ | ◐ real‑time availability formulation | 3 |
| 24 | SSR (Software Sensors) | gyro,accel,GPS,baro,mag | UAV+UGV | Detect+Recover | ✓ | ✓ | ✓ | ✓ | ✗ | 3 |
| 25 | SpecGuard | GPS,gyro,accel,cam,LiDAR | UAV+UGV | Recovery | ✓ | ◐ | ✗ | ✓ | ◐ STL mission specs | 3 |
| 26 | Stealthy vs CI/EKF | baro,gyro,accel,mag,GPS | UAV+UGV | Attack(evasion) | ◐ | ✓ | ✗ | ✗ | ✗ | 3 |
| 27 | TileMask | mmWave radar | AV/UGV | Attack | ✗ | ◐ | ✗ | ✗ | — | 3 |
| 28 | Un‑Rocking Drones | MEMS IMU | UAV | Attack+Recover | ✓ | ✗ | ◐ | ✓ | ✗ | 3 |

---

## Detailed answers

> Q1 Sensors · Q2 UAV/UGV · Q3 Model used to solve the problem · Q4 Attack types · Q5 Prediction · Q6 Threshold on prediction · Q7 Detection · Q8 Recovery · Q9 Formal guarantee (named) · Q10 Claiming strength · Q11 Novelty vs the prior works they cite · Q12 Limitation · Q13 Threat model in detail · Q14 Rating.
...
3. **Model (solution):** Formulates the attack as a constrained **optimization problem** `min L_adv(x⊕t′;M)`. Two functions are engineered: (a) an **input‑perturbation function** modeling spoofing capability via **global spatial transformations** (rotation/translation/scaling with a homography matrix + bilinear interpolation), and (b) an **adversarial objective** built from Apollo's post‑processing (objectness × positiveness, Gaussian mask at target). Because vanilla Adam optimization gets trapped in local minima, they add **global sampling + optimization** to raise success from 30%→~75%.
...
6. **Threshold:** ✓ — the learned formula's numeric bounds (e.g., 0.9634).
7. **Detection:** ✓ — core.
8. **Recovery:** ✗ (formal‑synthesis mitigation is future work).
...
5. **Prediction:** ✓ — historic‑state reconstruction/estimation.
6. **Threshold:** ✓ — detector + probabilistic diagnosis thresholds; computes max stealthy‑attack undetected duration.
7. **Detection:** ✓ — integrates existing detectors + adds independent diagnosis (masks detector false positives).
8. **Recovery:** ✓ — targeted per‑compromised‑sensor recovery.
9. **Formal guarantee:** ✗ (empirical).
...
3. **Model (solution):** Attack = acoustic **resonant‑frequency** excitation of the MEMS gyro, with two stealth injection methods (audio overlay) hiding ultrasound in ordinary sound; a **fuzzy‑PID** closed‑loop vehicle model in Carsim+Simulink evaluates impact. Defense = **LSTM + CUSUM** anomaly detector predicting yaw rate and substituting the predicted value on alarm.
4. **Attacks:** Covert acoustic gyro spoofing → understeer/oversteer, drift, rollover.
5. **Prediction:** ✓ (defense) — LSTM point‑by‑point yaw‑rate prediction.
6. **Threshold:** ✓ — CUSUM threshold `T` (average cumulative deviation over a window).
7. **Detection:** ✓ — LSTM‑CUSUM.
...
3. **Model (solution):** A **Nonlinear Unknown‑Input State Estimator (NUISE)** grounded in the robot's **kinematic physical‑dynamics model**. It predicts state evolution from planned control commands, compares predictions with measured readings, and cross‑validates sensors to **detect, pinpoint, and quantify** both sensor and actuator attack vectors — no majority voting or Byzantine threshold needed.
4. **Attacks:** Actuator + sensor attacks via signal interference, sensor spoofing, logic bomb, physical damage.
5. **Prediction:** ✓ — model‑based state prediction as ground truth.
6. **Threshold:** ✓ — **Chi‑square (χ²) test** thresholds at confidence levels (α=0.05 / 0.005).
7. **Detection:** ✓ — plus attack‑vector identification/forensics.
8. **Recovery:** ✗.
9. **Formal guarantee:** ◐ — **statistical guarantee via χ² hypothesis testing** on estimation residuals; formal system modeling but no security proof.
...
3. **Model (solution):** A **Generative Adversarial Network** whose Generator corrects/reconstructs attacked positions (preserving benign ones), chained to a **Deep RL** planner (instantiated as GAN‑DRL with an LSTM encoder for a variable obstacle count). Detector‑agnostic and method‑agnostic (drops into LSTM‑RL, SARL, ORCA).
4. **Attacks:** Localization Attacks (LAs) — GPS spoofing, IMU injection, LiDAR replay (modeled as p̃ = α·p).
5. **Prediction:** ✓ — Generator predicts potential positions.
6. **Threshold:** ✗ — deliberately detector‑agnostic (no threshold).
7. **Detection:** ✗ (by design, no detector).
8. **Recovery:** ✓ — mitigation via position correction + robust planning.
9. **Formal guarantee:** ✗ ("guarantee safe motion" is empirical).
...
3. **Model (solution):** A **hybrid formal + data pipeline**: derive **Safety Context Specifications in STL** from **control‑theoretic hazard analysis (STPA)**, then refine unknown thresholds via **(i) weakly‑supervised learning** and **(ii) an NN (MLP/LSTM) trained with a custom semantic loss** that penalizes violating the STL spec. Adds a **reaction‑time estimator** (stacked LSTM) predicting the time budget before a hazard.
4. **Attacks:** Accidental faults + malicious controller attacks; stealthy surge/bias attacks on control commands.
5. **Prediction:** ✓ — preemptive hazard prediction + reaction‑time budget.
6. **Threshold:** ✓ — STL thresholds β learned from data.
7. **Detection:** ✓ — context‑aware monitor flags unsafe control actions before hazards.
8. **Recovery:** ◐ — issues context‑specific corrective commands; reaction time scopes mitigation.
9. **Formal guarantee:** ◐ — **STL formal specification** (via STPA) integrated into training, but performance is empirical.
...
2. **Target:** Broad embedded/actuation systems (self‑balancing transporters, VR, Google‑Maps navigation, RC); not specifically UAV/UGV.
3. **Model (solution):** An **out‑of‑band signal‑injection model**: acoustic resonance is sampled by the ADC under an insufficient rate, so tiny **sample‑rate drift** is amplified. Two attack primitives — **SideSwing** (digital‑amplitude adjusting) and **Switching** (phase pacing via frequency switching) — give bidirectional control of the digitized output. An automatic attack program tunes frequency/threshold/target direction.
4. **Attacks:** Non‑invasive acoustic out‑of‑band injection to control inertial output in both directions.
5. **Prediction:** ✗.
6. **Threshold:** ✗ (attacker's own program uses a direction‑switch threshold).
7. **Detection:** ✗.
8. **Recovery:** ✗.
9. **Formal guarantee:** ◐ — a **theorem on sampling‑rate‑drift amplification** (signal‑processing result, not security).
10. **Claiming strength:** 23/25 devices affected, 17/25 implicitly controlled; non‑invasive; realistic consumer targets with demo videos.
11. **Novelty vs prior work:** Son et al. only caused gyro DoS; Trippel et al. (WALNUT) controlled *exposed* accelerometers on an Arduino in **one** direction and ignored ADC sample‑rate drift. This work achieves **bidirectional control of *embedded* sensors non‑invasively**, explicitly modeling sample‑rate drift, amplitude adjusting, and phase pacing.
12. **Limitation:** Range ~several meters; must profile an identical device; digital frequency pattern sometimes unpredictable.
13. **Threat model (detail):** Goal = induce specific digital sensor outputs to trigger actuations. Capability = non‑invasive acoustic injection, no physical contact, no access to the sampling process; profiles the behavior of an *identical* device offline; sufficient funds/technique; ~meters range.
14. **Rating:** **3/3**.
...
7. **Detection:** ✗ (defeats it; evaluates DNN defenses).
8. **Recovery:** ✗.
...
3. **Model (solution):** Two **sequence‑to‑sequence (encoder‑decoder RNN) models**: (1) a **state‑estimation Seq2Seq** capturing temporal/spatial dependencies among sensors + control demands to reconstruct states from trustworthy history; (2) a **recovery‑control Seq2Seq** that, MPC‑style, uses remaining intact sensors + a target state to emit a *sequence* of control demands. Model‑free (no system dynamics).
4. **Attacks:** Sensor spoofing/transduction — GPS spoofing, acoustic on accelerometer.
5. **Prediction:** ✓ — core (sequence‑predictive estimation + control).
6. **Threshold:** ✗ — assumes external detector; recovery ends at target state.
7. **Detection:** ✗ — assumes an existing detector (with delay).
8. **Recovery:** ✓ — core.
9. **Formal guarantee:** ✗ (learning‑based).
...
3. **Model (solution):** A **machine‑learning Feed‑Forward Controller (FFC)** — an **LSTM** trained with **feature engineering** (to break PID input collinearity) that predicts the PID controller's *output* under attack‑free conditions. It runs in tandem with the PID; when the PID‑vs‑FFC deviation exceeds threshold τ it flags an attack and the FFC takes over until the attack subsides.
4. **Attacks:** Physical attacks — GPS spoofing, acoustic gyro/accelerometer; overt + stealthy.
5. **Prediction:** ✓ — FFC predicts controller output.
6. **Threshold:** ✓ — deviation threshold τ (tight, from a precise ML model → limits stealthy attacks).
7. **Detection:** ✓ — via PID‑FFC divergence.
8. **Recovery:** ✓ — FFC drives the RV.
9. **Formal guarantee:** ✗.
...
3. **Model (solution):** **Physical challenge‑response authentication**: the sensor's actuator emits a *random but deliberate* physical probe and briefly turns off; because of **physical passivity**, an attacker's injected signal cannot instantly vanish. A **χ² detector** built on a real‑time **system model** predicts the expected response and flags residuals during the "actuator‑off" window; attack‑resilient estimation mitigates.
4. **Attacks:** Physical spoofing of active sensors (analog domain), plus eavesdropping; simple vs advanced spoofing.
5. **Prediction:** ✓ — models/predicts expected sensor output.
6. **Threshold:** ✓ — χ² alarm threshold α (tuned to noise).
7. **Detection:** ✓ — authentication core.
8. **Recovery:** ◐ — attack‑resilient estimation.
9. **Formal guarantee:** ✓ — **Theorem 1**: the attacker's mis‑detection delay τ_attack ≥ τ_physical_limit > 0 is **provably bounded away from zero** (rooted in change‑point‑detection limits on noisy signals). Method = **information‑theoretic / change‑point‑detection bound** + χ² detector.
...
3. **Model (solution):** A **Linear‑Quadratic Regulator (LQR)** recovery controller with timing + safety constraints, whose horizon = safety deadline + a maintainable time; solved fast by an **ADMM** algorithm. Supporting components: a **sliding‑window checkpointer** (trustworthy data under variable detection delay), a **state reconstructor**, and a **reachability‑based deadline estimator**.
4. **Attacks:** Sensor/transduction attacks (3 types).
5. **Prediction:** ✓ — state reconstruction + reachability.
6. **Threshold:** ✗ — no detection threshold (assumes detector); uses a safety deadline.
7. **Detection:** ✗ — assumes an existing detector.
8. **Recovery:** ✓ — core.
9. **Formal guarantee:** ✓ — **reachability analysis** guarantees no unsafe state is reachable during recovery and that the system reaches/maintains the target set before the deadline.
...
3. **Model (solution):** A **deviation monitor** (segment deviation over consecutive intervals) + a **reinforcement‑learning intelligent agent** (continuous‑control, DDPG‑style) that generates a proper configuration for the current flight condition, iterating reconfiguration until instability is eliminated.
4. **Attacks:** **Not adversarial** — misconfigurations (improper configs / inappropriate settings) causing instability.
5. **Prediction:** ◐ — the agent generates configs from state; deviation monitoring.
6. **Threshold:** ✓ — instability threshold TH on the segment deviation value.
7. **Detection:** ✓ — instability detection (F1 99.51% ArduPilot).
8. **Recovery:** ✓ — on‑the‑fly reconfiguration (state rectifier).
9. **Formal guarantee:** ✗.
...
3. **Model (solution):** Four components — a **state predictor** doing **nonlinear reachability analysis** (via **Flow\*** / Taylor models) to reconstruct the recovery initial‑state set; a **time oracle** computing an online safety deadline; a **model adaptor** that linearizes the nonlinear dynamics on the fly; and an **MPC‑style adaptive recovery‑sequence generator** solving the linearized problem. Crucially it keeps using **uncompromised sensors** to curb uncertainty accumulation.
4. **Attacks:** Sensor/transduction attacks (GPS spoofing, camera light injection).
5. **Prediction:** ✓ — state predictor (data‑predictive).
6. **Threshold:** ✗ — assumes a detector; uses an online safety deadline.
7. **Detection:** ✗ — assumes existing detector/diagnosis.
8. **Recovery:** ✓ — core.
9. **Formal guarantee:** ✓ — **nonlinear reachability analysis (Flow\*)** guarantees the CPS never reaches unsafe states, recovers before the deadline, and stays in the target set for a period; a fail‑safe takes over if the safety check fails.
...
3. **Model (solution):** A **taxonomy + qualitative risk assessment** of vulnerabilities/attacks/countermeasures across physical/network layers, plus authentication/crypto recommendations and a COVID‑19 robotics section.
...
6. **Threshold:** N/A.
7. **Detection:** ✓ — extensive IDS/IPS/honeypot review.
8. **Recovery:** ◐ — mentions RV recovery (e.g., Choi et al.).
...
3. **Model (solution):** A **stacked Denoising AutoEncoder (DAE)** that restores real obstacle positions from corrupted ones (encoder takes current corrupted position + recorded previous position/velocity; decoder outputs the approximate true position), feeding an **LSTM‑based Deep‑RL planner (Crit‑LSTM)** that handles a variable obstacle count. Detector‑ and method‑agnostic.
...
5. **Prediction:** ✓ — DAE estimates real positions; predicts obstacle states.
6. **Threshold:** ✗ (detector‑agnostic).
7. **Detection:** ✗ (by design).
8. **Recovery:** ✓ — mitigation via position correction.
9. **Formal guarantee:** ✗ (empirical "guarantee safe motion").
...
3. **Model (solution):** Evaluates **ML anomaly detectors (MLP, LSTM)** and augments them with **domain knowledge as a semantic loss** — encoding **STL** unsafe‑control‑action formulas (from STPA hazard analysis) as a regularization term during training.
...
5. **Prediction:** ✓ — hazard prediction.
6. **Threshold:** ◐ — classification (implicit).
7. **Detection:** ✓ — ML safety monitoring, robustness‑tested.
8. **Recovery:** ✗.
9. **Formal guarantee:** ✗ — STL knowledge integrated, but empirical.
...
3. **Model (solution):** **Static program analysis** identifies **Safety‑Critical Variables (SCVs)** in the control program, then an **s‑t‑cut on the data‑flow graph** selects a minimal **monitored set (mSCVs)** that all input changes must flow through, scored by a **recovery‑suitability** criterion. Detection = two thresholds on mSCV change; recovery = replace the offending mSCV with a predicted normal value (avg of previous values) + mode switching.
4. **Attacks:** Data‑oriented attacks — **sensor** attacks + **parameter** attacks (20 types), incl. stealthy and combined.
5. **Prediction:** ✓ — predicts normal mSCV values.
6. **Threshold:** ✓ — Threshold1 (single‑cycle diff) + Threshold2 (accumulated diff over a window).
7. **Detection:** ✓ — mSCV monitoring.
8. **Recovery:** ✓ — value replacement + normal/recovery mode switch.
9. **Formal guarantee:** ◐ — **proof (mathematical induction) that the s‑t‑cut mSCV set is "complete"** (every source→sink data‑flow path passes through an mSCV); not a security proof.
...
3. **Model (solution):** **Vertically‑integrated, language‑based information‑flow control**: the planner + map‑verifier are written in **Jif** (a security‑typed language enforcing integrity/confidentiality labels at compile time) so only verified map data reaches the planner; a **formally‑verified secure processor (HyperFlow)** enforces timing‑safe isolation in hardware; a map‑verification algorithm cross‑checks external maps against local sensors.
4. **Attacks:** Remote software attacks (compromised map server), sensor spoofing, hardware timing interference.
5. **Prediction:** ✗ (input verification, not state prediction).
6. **Threshold:** ✗.
7. **Detection:** ✓ — input verification via local sensors detects adversarial map manipulation.
8. **Recovery:** ✗ (prevention/verification; reviews restart‑based recovery).
9. **Formal guarantee:** ✓ — **provable end‑to‑end security via static information‑flow typing (Jif) + formally‑verified hardware (HyperFlow, SecVerilog‑style)**; timing‑sensitive noninterference.
...
3. **Model (solution):** An **ML multi‑classifier** (primarily an **MLP**, vs decision tree/RF/XGBoost) trained on **heterogeneous data fusion** (inertial + baro + GPS + EKF/filter parameters) with **feature‑importance / correlation / ablation** feature selection; deployed on firmware via **frugally‑deep** for online inference.
4. **Attacks:** Sensor injection attacks — gyro, accelerometer, barometer, GPS.
5. **Prediction:** ◐ — classification (not state prediction).
6. **Threshold:** ◐ — classification confidence (implicit).
7. **Detection:** ✓ — detection + **attack‑type classification**.
...
5. **Prediction:** ✓ — software sensor predicts the real reading.
6. **Threshold:** ✓ — recovery‑switch threshold `T_on` + recovery‑off threshold; accumulated‑error window.
7. **Detection:** ✓ — via discrepancy (recovery switch).
8. **Recovery:** ✓ — software sensors replace compromised ones.
9. **Formal guarantee:** ✗.
...
6. **Threshold:** ✓ — central to the attack; learns detection thresholds/windows to stay under them (exploiting the large thresholds set at mode changes).
7. **Detection:** ✗ — this is an **evasion** paper (defeats detection).
8. **Recovery:** ✗.
9. **Formal guarantee:** ✗.
